(() => {
  ...
  const POLICY_YEAR = new Date().getFullYear();
  ...
  qs('#motorForm')?.addEventListener('submit', (e) => {
    ...
    if (data.product === 'tplPrivate') {
      ...
      const mdLimits = { none: 'No material damage extension', '1': '$500,000 material damage limit', '2': '$750,000 material damage limit', '3': '$1,000,000 material damage limit' };
      const bi = 45;
      const md = mdPremiums[data.tplOption];
      const total = bi + md;
      renderResult('#motorResult', { title: 'Private Car TPL estimate', total, rows: [['Bodily injury premium', money(bi)], ['Material damage option', mdLimits[data.tplOption]], ['Material damage premium', money(md)], ['Total', money(total)]], notes, whatsappText: baseMessage('Motor TPL - Private Car', [`Client: ${data.clientName || 'Not provided'}`, `Vehicle: ${data.vehicle}`, `Material damage option: ${mdLimits[data.tplOption]}`], total, notes.map((n) => n.text)) });
      return;
    }
    ...
    if (data.product === 'otherTpl') {
      ...
      renderResult('#motorResult', { title: `${other.label} TPL estimate`, total: other.total, rows: [['Category', other.label], ['Bodily injury premium', money(other.bi)], ['Material damage premium', money(other.md)], ['Total', money(other.total)]], notes: allNotes, whatsappText: baseMessage('Motor TPL - Other Vehicle', [`Client: ${data.clientName || 'Not provided'}`, `Vehicle: ${data.vehicle}`, `Category: ${other.label}`, `Metric: ${data.otherMetric || 'Not provided'}`, `Material damage limit: ${data.otherMaterialLimit}`], other.total, allNotes.map((n) => n.text)) });
    }
  });
