(() => {
  ...
  const medicalPlans = {
    ...
    ultra: {
      ...
      inHospital: {
        individual: {
          '14d-17': { A: 760, B: 650, S: 550 }, '18-24': { A: 1000, B: 720, S: 670 }, '25-30': { A: 1360, B: 1120, S: 910 }, '31-35': { A: 1700, B: 1180, S: 1100 }, '36-40': { A: 1850, B: 1250, S: 1120 }, '41-45': { A: 1950, B: 1350, S: 1150 }, '46-50': { A: 2100, B: 1550, S: 1300 }, '51-55': { A: 2800, B: 1800, S: 1560 }, '56-60': { A: 3750, B: 2500, S: 2050 }, '61-64': { A: 4920, B: 3400, S: 2950 }, '65-70': { A: 7000, B: 5200, S: 3820 }, '71-75': { A: 9800, B: 5950, S: 4600 }, '76-80': { A: 11000, B: 8000, S: 5800 }, '81+': { A: 14500, B: 9300, S: 7500 }
        },
        family: {
          '14d-17': { A: 699, B: 598, S: 506 }, '18-24': { A: 920, B: 662, S: 616 }, '25-30': { A: 1251, B: 1030, S: 837 }, '31-35': { A: 1564, B: 1086, S: 1012 }, '36-40': { A: 1702, B: 1150, S: 1030 }, '41-45': { A: 1794, B: 1242, S: 1058 }, '46-50': { A: 1932, B: 1426, S: 1196 }, '51-55': { A: 2576, B: 1656, S: 1435 }, '56-60': { A: 3450, B: 2300, S: 1886 }, '61-64': { A: 4526, B: 3128, S: 2714 }, '65-70': { A: 6440, B: 4784, S: 3514 }, '71-75': { A: 9016, B: 5474, S: 4232 }, '76-80': { A: 10120, B: 7360, S: 5336 }, '81+': { A: 13340, B: 8556, S: 6900 }
        }
      },
      ambRates: {
  ...
  function setupMedical() {
    ...
    const memberCount = qs('#medicalMemberCount');
    const members = qs('#medicalMembers');
    if (!plan) return;
    plan.innerHTML = optionMarkup(Object.entries(medicalPlans).map(([value, p]) => ({ value, label: p.label })));

    function memberRowHtml(i, classes) {
      const classOptions = optionMarkup(classes.map((value) => ({ value, label: `Class ${value}` })));
      return `
        <div class="field medical-member-row" data-member-row="${i}">
          <label>Member ${i} name / relation</label>
          <input name="memberName${i}" type="text" placeholder="Example: Father / Child" />
        </div>
        <div class="field medical-member-row" data-member-row="${i}">
          <label>Member ${i} year of birth</label>
          <input name="memberYob${i}" type="number" min="1920" max="2026" placeholder="Example: 1983" />
        </div>
        <div class="field medical-member-row" data-member-row="${i}">
          <label>Member ${i} age</label>
          <input name="memberAge${i}" type="number" min="0" max="99" placeholder="Auto from YOB or type age" />
        </div>
        <div class="field medical-member-row" data-member-row="${i}">
          <label>Member ${i} hospital class</label>
          <select name="memberClass${i}">${classOptions}</select>
        </div>`;
    }

    function refreshMembers() {
      const p = medicalPlans[plan.value];
      const count = Number(memberCount?.value || 1);
      members.innerHTML = Array.from({ length: count }, (_, idx) => memberRowHtml(idx + 1, p.classes)).join('');
    }
    ...
    cat.addEventListener('change', refreshMembers);
    memberCount?.addEventListener('change', refreshMembers);
    refresh();
  }
  ...
  qs('#medicalForm')?.addEventListener('submit', (e) => {
    ...
    for (let i = 1; i <= memberCount; i++) {
      const memberName = data[`memberName${i}`] || `Member ${i}`;
      const yob = num(data[`memberYob${i}`]);
      const typedAge = num(data[`memberAge${i}`]);
      const age = typedAge || (yob ? Math.max(0, POLICY_YEAR - yob) : 0);
      const memberClass = data[`memberClass${i}`] || p.classes[0];
      const band = bandFor(age, p.bands);
      if (!band) {
        hasBlockingIssue = true;
        members.push({ i, memberName, age, yob, memberClass, bandLabel: 'Outside tariff bands', inHospital: 0, ambulatory: 0, issue: 'Age outside supported tariff bands.' });
        continue;
      }

      const inTable = p.inHospital[data.category] || p.inHospital;
      const inHospital = inTable?.[band.key]?.[memberClass];
      if (inHospital === undefined) {
        hasBlockingIssue = true;
        members.push({ i, memberName, age, yob, memberClass, bandLabel: band.label, inHospital: 0, ambulatory: 0, issue: 'Combination not available in the selected tariff.' });
        continue;
      }
      ...
      totalAmbulatory += ambRate;
      members.push({ i, memberName, age, yob, memberClass, bandLabel: band.label, inHospital, ambulatory: ambRate });

      if (age > 64 && !['youth'].includes(data.plan)) notes.push({ type: 'warning', text: `${memberName}: maximum first enrollment age is generally 64; older ages may require company approval or continuity.` });
      if (data.plan === 'youth' && (age < 18 || age > 35)) notes.push({ type: 'warning', text: `${memberName}: Panacea Youth is designed for ages 18 to 35 only.` });
    }

    if (hasBlockingIssue) {
      const rows = members.map((m) => [`${m.memberName}`, `${m.issue || 'OK'} Age: ${m.age || 'N/A'}, Class: ${m.memberClass}`]);
      renderReferral('#medicalResult', 'Medical quote needs review', rows, notes, baseMessage('Medical Insurance', [`Policyholder/family: ${data.clientName || 'Not provided'}`, `Plan: ${p.label}`, `Category: ${categoryLabels[data.category]}`, `Members: ${memberCount}`], 0, notes.map((n) => n.text)));
      return;
    }

    const cost = p.cost || 0;
    const total = totalInHospital + totalAmbulatory + cost;
    if (data.category === 'family' && memberCount < 3) notes.push({ type: 'warning', text: 'Family tariff is intended for 3 members and above. Please confirm eligibility before issuance.' });
    ...
    notes.push({ text: 'Medical tariff updated to V13 effective August 1, 2026. Cost of policy is added once per quotation, not once per member.' });
    ...
    const memberRows = members.flatMap((m) => [
      [`${m.memberName} - IN`, `${money(m.inHospital)} (${m.bandLabel}, Class ${m.memberClass}, age ${m.age})`],
      [`${m.memberName} - Ambulatory`, money(m.ambulatory)]
    ]);
    const details = [
      `Policyholder/family: ${data.clientName || 'Not provided'}`,
      `Plan: ${p.label}`,
      `Category: ${categoryLabels[data.category]}`,
      `Ambulatory: ${ambLabels[data.ambulatory]}`,
      ...members.map((m) => `${m.memberName}: age ${m.age}, YOB ${m.yob || 'not provided'}, Class ${m.memberClass}, IN ${money(m.inHospital)}, AMB ${money(m.ambulatory)}`)
    ];
    renderResult('#medicalResult', {
      title: `${p.label} ${memberCount > 1 ? 'multi-member' : 'single-member'} estimate`, total,
      rows: [['Members quoted', memberCount], ['In-hospital total', money(totalInHospital)], ['Ambulatory total', money(totalAmbulatory)], ['Cost of policy', money(cost)], ...memberRows, ['Total yearly premium', money(total)]],
      notes,
  ...
  qs('#motorForm')?.addEventListener('submit', (e) => {
    ...
    if (data.product === 'tplPrivate') {
      ...
      const mdLimits = { none: 'No Material Damage option selected', '1': 'MMD Option 1 - $500,000 annual limit', '2': 'MMD Option 2 - $750,000 annual limit', '3': 'MMD Option 3 - $1,000,000 annual limit' };
      const mdBenefits = { none: 'No MMD benefits selected', '1': 'Accident-only road assistance, family medical expenses $2,000, death/PTD $4,000', '2': 'Accident road assistance + 1 mechanical/electrical ride up to 75 km, family medical expenses $2,500, death/PTD $5,000', '3': 'Accident road assistance + 2 mechanical/electrical rides, family medical expenses $5,000, death/PTD $10,000' };
      const biSelected = data.compulsory === 'on';
      const bi = biSelected ? 45 : 0;
      const md = mdPremiums[data.tplOption];
      const total = bi + md;
      if (biSelected) notes.push({ text: 'Bodily Injury TPL premium added as a separate policy line: $45 for private cars.' });
      if (data.tplOption !== 'none') notes.push({ text: 'MMD covers material damages to third parties up to the selected annual limit, including fire/explosion of the insured vehicle up to the same chosen limit.' });
      if (!biSelected && data.tplOption === 'none') notes.push({ type: 'warning', text: 'No Bodily Injury TPL or Material Damage option was selected, so the total is $0.' });
      renderResult('#motorResult', { title: 'Private Car Material Damage estimate', total, rows: [['Bodily Injury TPL selected', biSelected ? 'Yes' : 'No'], ['Bodily Injury TPL premium', money(bi)], ['MMD / Motor Material Damage option', mdLimits[data.tplOption]], ['MMD premium', money(md)], ['Option benefits', mdBenefits[data.tplOption]], ['Total', money(total)]], notes, whatsappText: baseMessage('Motor Third Party Material Damage - Private Car', [`Client: ${data.clientName || 'Not provided'}`, `Vehicle: ${data.vehicle}`, `Bodily Injury TPL selected: ${biSelected ? 'Yes' : 'No'}`, `MMD option: ${mdLimits[data.tplOption]}`, `Option benefits: ${mdBenefits[data.tplOption]}`], total, notes.map((n) => n.text)) });
      return;
    }
    ...
    if (data.product === 'otherTpl') {
      ...
      const biSelected = data.compulsory === 'on';
      const biPremium = biSelected ? other.bi : 0;
      const total = biPremium + other.md + roadAssistance;
      ...
      if (biSelected) allNotes.push({ text: `Bodily Injury TPL premium added as a separate policy line for this category: ${money(other.bi)}.` });
      ...
      if (!biSelected && other.md === 0) allNotes.push({ type: 'warning', text: 'No Bodily Injury TPL or Material Damage option was selected, so the total is $0.' });
      renderResult('#motorResult', { title: `${other.label} Material Damage estimate`, total, rows: [['Category', other.label], ['Bodily Injury TPL selected', biSelected ? 'Yes' : 'No'], ['Bodily Injury TPL premium', money(biPremium)], ['MMD / Motor Material Damage limit', data.otherMaterialLimit === 'none' ? 'No Material Damage option selected' : money(data.otherMaterialLimit)], ['MMD premium', money(other.md)], ['Optional road assistance', money(roadAssistance)], ['Total', money(total)]], notes: allNotes, whatsappText: baseMessage('Motor Third Party Material Damage - Other Vehicle', [`Client: ${data.clientName || 'Not provided'}`, `Vehicle: ${data.vehicle}`, `Category: ${other.label}`, `Metric: ${data.otherMetric || 'Not provided'}`, `Bodily Injury TPL selected: ${biSelected ? 'Yes' : 'No'}`, `MMD limit: ${data.otherMaterialLimit}`, `Optional road assistance: ${roadAssistance ? 'Yes' : 'No'}`], total, allNotes.map((n) => n.text)) });
    }
  });
  ...
  qs('#schoolForm')?.addEventListener('submit', (e) => {
    e.preventDefault();
    const data = getForm(e.currentTarget); const persons = num(data.students) + num(data.teachers); const rate = data.option === '1' ? 5 : 10; const calc = persons * rate; const total = Math.max(calc, 750);
    const notes = [];
    if (total === 750) notes.push({ text: 'Minimum annual premium of $750 applied.' });
    notes.push({ text: 'School Liability V2 effective September 1, 2026. Coverage applies inside school premises and during school-sponsored activities outside the premises such as trips and events, excluding summer camps.' });
    notes.push({ text: 'Hospitalization is 3rd class. All hospitals excluding AUBMC, CMC, BVMC, KMC, St. John, and Rizk.' });
    notes.push({ text: 'Passive War cover applies excluding listed Red Zones. Required documents: list of students, list of teachers, and any additional document requested case-by-case.' });
    const limits = data.option === '1' ? 'Death/PTD $25,000 per person, Medical Expenses $2,500 per person, Aggregate $150,000' : 'Death/PTD $50,000 per person, Medical Expenses $5,000 per person, Aggregate $300,000';
    renderResult('#schoolResult', { title: 'School Liability V2 estimate', total, rows: [['Students', data.students], ['Teachers', data.teachers], ['Total insured persons', persons], ['Rate per person', money(rate)], ['Calculated premium', money(calc)], ['Limits', limits], ['Total after minimum check', money(total)]], notes, whatsappText: baseMessage('School Liability', [`School/camp: ${data.schoolName || 'Not provided'}`, `Students: ${data.students}`, `Teachers: ${data.teachers}`, `Option: ${data.option}`, `Limits: ${limits}`], total, notes.map((n) => n.text)) });
  });
